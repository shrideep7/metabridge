...
function openModPicker(report, row) {
  ...
  // Every class the MANIFEST can carry as logic has to be listed here, or the
  // filter below removes what was never offered: the manifest arrives with the
  // object in it, nothing selects it, and it is stripped on the way out. That
  // is what happened to Teradata MACROs — the estate showed them, the manifest
  // shipped them, and the picker deleted every one. Keep in step with
  // procedures.ANALYSIS_KINDS.
  const logic = [].concat(report.procedures || [], report.macros || [],
                          report.packages || []);
  const objs = tables.map(t => ({schema: t.schema || '', name: t.name,
    ...
    .concat(logic.map(p => ({schema: p.schema || '', name: p.name,
                             kind: 'procedure', label: p.kind || 'procedure'})));
  ...
  $('#modObjList').innerHTML = shown.length
    ? shown.map(o => {
        ...
        return '<label style="display:flex;gap:8px;align-items:center;padding:5px 12px;'
          ...
          + (o.kind === 'view' ? 'view'
             : o.kind === 'procedure' ? (o.label || 'procedure')
...
async function fillScaffoldSelects() {
  ...
  $('#scafTarget').innerHTML = groupedConnectorOptions(
    ...
    'snowflake');
  // This runs on every page LOAD, so the hardcoded snowflake default silently
  // undid a target the user had already chosen — and the Modernize flow sets
  // only the source, so the next generate produced Snowflake artifacts for a
  // migration aimed at Databricks. Remember the choice instead of asking for
  // it twice. Same localStorage idiom the Modernize filters already use.
  const savedTarget = localStorage.getItem('mb_scaf_target');
  if (savedTarget && [...$('#scafTarget').options].some(o => o.value === savedTarget))
    $('#scafTarget').value = savedTarget;
...
$('#scafTarget').onchange = () => {
  localStorage.setItem('mb_scaf_target', $('#scafTarget').value);
  syncMvChips();
};
...
function estateAssets(d) {
  ...
  // A Teradata MACRO is a parameterized SQL statement — real logic,
  // returned by the backend and rendered by nothing until now.
  push(d.macros, 'Macro');
...
// count pills.
// The PREFERRED ORDER of the type filter, no longer the set of types that may
// appear: anything the estate actually contains gets a chip (see below). This
// list had drifted 14 types behind what the asset table already rendered —
// packages, triggers, synonyms, macros and the rest were rows nobody could
// filter to, and the chip row understated the estate.
const ESTATE_TYPES = ['Table', 'View', 'Materialized view', 'Dynamic table',
  'Sequence', 'File format', 'Function', 'Stored procedure', 'Macro',
  'Package', 'Trigger', 'Index', 'Constraint', 'Synonym', 'Database link',
  'Queue', 'Object type', 'Rule', 'Stream', 'Task', 'Pipe', 'Stage', 'Volume',
  'Scheduler job', 'Scheduler program', 'Schedule', 'Scheduler chain',
  'Masking policy', 'Row access policy', 'Tag', 'Share'];
// "Masking policy" pluralises to "policies", "Index" to "indexes"
const estatePlural = t => t.endsWith('s') ? t
  : t.endsWith('y') ? t.slice(0, -1) + 'ies'
  : /(x|ch|sh|z)$/.test(t) ? t + 'es' : t + 's';
...
function renderEstateContext(d) {
  ...
  // Known types first, in the curated order; then ANY other type present in
  // the data. A hardcoded list is a place for a class to go missing, and this
  // is the third one that did — so the data decides what is filterable and the
  // list only decides what comes first. Grants already have their own count
  // pill below and would otherwise appear twice.
  const shown = ESTATE_TYPES.filter(t => byType[t] || active === t);
  Object.keys(byType).sort().forEach(t => {
    if (shown.indexOf(t) === -1 && t !== 'Grant') shown.push(t);
  });
  const pills = pill('All', '', scoped.length, !active, true)
    + shown.map(t => pill(byType[t] === 1 ? t : estatePlural(t), t,
                          byType[t] || 0, active === t, true)).join('')
