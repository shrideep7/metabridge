...
def generate_kafka(cer: CER, flavor: str = "kafka",
    ...
    if cer.schemas:
        ...
        # One entry per SUBJECT, newest version. Iterating cer.schemas
        # (the version history) emitted one file and one POST per version:
        # every version of a subject collided on the same _ident filename
        # so only the last-written survived, and the registry received the
        # versions in filename order, leaving whichever sorted last as the
        # newest registered version. Both silently discarded the real
        # current schema.
        for s in cer.current_schemas():
            ...
            superseded = [v.version for v in cer.schema_versions(s.name)
                          if v.version != s.version]
            if superseded:
                reg.append("# subject %s: source carried version(s) %s as "
                           "well — registering v%d only, the current one"
                           % (subject, ", ".join("v%d" % v for v in
                                                 superseded), s.version))
            if s.compatibility:
    ...
    for cdc in cer.cdc_sources:
        cfgd = {"connector.class":
                ...
                # The source prefix, not the connector name. Emitting
                # cdc.name renamed every generated topic away from the
                # CER channels the rest of the output references, so
                # consumers subscribing by name would find nothing.
                "topic.prefix": cdc.topic_prefix or cdc.name,
...
def generate_ksql(cer: CER) -> str:
    ...
    # Source declarations first: a job's FROM clause names a stream that
    # ksqlDB requires to already exist. Emitting only the jobs produced a
    # script whose first statement referenced an undeclared stream.
    for d in cer.declarations:
        stmt = (d.raw or "").strip().rstrip(";")
        lines.append((stmt + ";") if stmt else
                     "CREATE %s %s WITH (KAFKA_TOPIC='%s');"
                     % (d.kind.upper(), _ident(d.name),
                        d.topic or d.name))
    for t in cer.transformations:
        ...
        if t.window is not None:
            ...
            # The clause is emitted from the STRIPPED body, so the re-add
            # guard has to test the stripped text too. Testing `body` saw
            # the GROUP BY that _strip_group was about to remove, decided
            # one was already present, and emitted an aggregate with a
            # bare non-aggregated column — invalid ksqlDB.
            stripped = _strip_group(body)
            grp = (" GROUP BY " + ", ".join(t.group_by)) \
                if t.group_by and "GROUP BY" not in stripped.upper() else ""
            lines.append(
                "CREATE TABLE %s WITH (KAFKA_TOPIC='%s') AS\n  %s\n"
                "  WINDOW %s%s EMIT CHANGES;"
                % (_ident(t.name), t.output or t.name,
                   stripped, win, grp))
            if w.time_semantics in ("event_time", "unknown") and \
...
def generate_flink(cer: CER, rep: Optional[GenReport] = None) -> str:
    ...
    lines = ["-- Flink SQL — generated by MetaBridge AI from %s"
             % cer.source_platform]
    # current_schemas(), not schemas: keying off the version history gave
    # each column the type from whichever version sorted last, so a table
    # could be declared with a superseded field type.
    schema_of = {s.name: s for s in cer.current_schemas()}
    for ch in cer.channels:
    ...
    for t in cer.transformations:
        ...
        if t.window is None:
            ...
            unresolved = [i for i in t.inputs
                          if not any(c.name == cer.resolve_stream(i)
                                     for c in cer.channels)]
            if unresolved:
        ...
        if not w.watermark_delay_ms and \
                w.time_semantics in ("event_time", "unknown"):
